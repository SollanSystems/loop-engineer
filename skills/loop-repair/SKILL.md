...
- **No editing the tests, fixtures, golden files, rubric, or `SPEC.md` to manufacture a pass.** Fix the code, not the goalposts. The verifier is a protected, *independent* set; modifying it during a run is itself a verifier-gaming signal → hard-terminate `FailedSafety` + logged as a security failure. This is the single hardest rule in the skill.
